...
(defn gl-window
  ...
  (let [display (NewtFactory/createDisplay nil)
    ...
    (doto win
      ...
      (.setFullscreen (get opts :fullscreen false))
      (.setPointerVisible (get opts :pointer-visible true))
      (.confinePointer (get opts :pointer-locked false))
      (.setTitle (get opts :title "GLWindow"))
      (.setVisible (get opts :visible true))
      (.addGLEventListener (gl-event-proxy (get opts :events))))
    (when-let [m (get-in opts [:events :mouse])]
      (.addMouseListener win (mouse-proxy m)))
...
(defn double-buffer
  [n-or-coll]
  (if (number? n-or-coll)
    (GLBuffers/newDirectDoubleBuffer n-or-coll)
...
void main() { outCol = col; //vec4(1.0, 0.0, 0.0, 1.0);
}"})
...
(defn display
  ...
  (let [^GL3 gl (-> drawable (.getGL) (.getGL3))
    ...
    (doto gl
      ...
      (.glUniform1f (get-in unis [:time :loc]) (float t))
